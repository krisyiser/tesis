...
import { Menu, X, Sun, User, Map as MapIcon, ChevronDown } from "lucide-react";
  ...
  return (
    <nav 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${
        scrolled ? "bg-black/95 backdrop-blur-xl py-4 shadow-2xl border-b border-white/5" : "bg-transparent py-8"
      }`}
    >
      <div className="max-w-[1600px] mx-auto px-8 flex items-center justify-between">
        ...
        {/* CENTER LOGO - PUEBLA STYLE */}
        <Link href="/" className="flex flex-col items-center px-10 group">
          <motion.div 
            animate={scrolled ? { scale: 0.8 } : { scale: 1 }}
            className="flex flex-col items-center"
          >
            <span className="text-3xl font-black text-white tracking-[0.2em] leading-none text-center">PAPANTLA</span>
        ...
        <div className="hidden lg:flex items-center justify-end gap-8 flex-1">
          ...
          <div className="flex items-center gap-5">
            <button className="text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-1 hover:text-primary transition-colors">
              ES <ChevronDown className="w-3 h-3" />
            </button>
            <button className="text-white/60 hover:text-primary transition-all hover:scale-110"><Sun className="w-4 h-4" /></button>
            <button className="text-white/60 hover:text-primary transition-all hover:scale-110"><User className="w-4 h-4" /></button>
          </div>
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-full left-0 right-0 bg-black/98 backdrop-blur-3xl border-t border-white/10 p-10 flex flex-col items-center gap-8 lg:hidden shadow-2xl"
          >
            {[
              { name: "Inicio", path: "/" },
              ...
              <Link 
                ...
                className={`text-2xl font-black uppercase tracking-[0.2em] ${pathname === item.path ? "text-primary" : "text-white"}`}
              >
                {item.name}
              </Link>
            ))}
